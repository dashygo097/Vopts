...
select_vcd() {
  ...
  local vcd_file=$(find "$SIM_DIR" -type f -name "*.vcd" | sed "s|^$TB_DIR/obj_dir||" | fzf --height=40% --prompt="Fuzzy Search: " --header="Use arrow keys to navigate, Enter to select")
...
run_test() {
  ...
  verilator --quiet --cc --exe --build --binary --trace -Wno-WIDTHEXPAND -Wno-WIDTHTRUNC "$tb_name" -o "${tb_name%.*}" -Mdir "$SIM_DIR" > "$LOG_DIR/tb.log" 2>&1
