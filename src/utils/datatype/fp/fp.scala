...
object FP {
  ...
  def apply(dw: Int, bp: Int, value: UInt): FP   =
    FP(dw, bp).fromUInt(value)
  def apply(dw: Int, bp: Int, value: SInt): FP   =
    FP(dw, bp).fromSInt(value)
}
...
trait FPOps {
  ...
  def +(that: UInt): FP   = this + FP(dw(), bp(), that)
  ...
  def <(that: UInt): Bool   = this < FP(dw(), bp(), that)
