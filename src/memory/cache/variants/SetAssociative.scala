  ...
  switch(state) {
    ...
    is(CacheState.IDLE) {
      ...
      when(upper.req.fire) {
        ...
        val useForwardedData = CombTree.andTree(
          Seq(
            lastWriteValid,
    ...
    is(CacheState.WRITEBACK) {
      ...
      when(memReqSent && lower.resp.fire) {
        // Assuming single beat for lower writeback, or waiting for lower's last if multi-beat were implemented
