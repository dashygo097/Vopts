...
class Matrix[T <: Data](gen: T, row: Int, col: Int)(implicit ev: Arithmetic[T]) extends Bundle with MatrixOps[T] {
  ...
  def apply(r: Int, c: Int): T = {
    require(r >= 0 && r < _row && c >= 0 && c < _col, s"Matrix index out of bounds: ($r, $c), size=(${_row}, ${_col})")
    value(r)(c)
  }
  ...
  def fromSeq(seq: Seq[Seq[T]]): Matrix[T] = {
    require(seq.length == _row && seq.forall(_.length == _col),
  ...
  def slice(rowStart: Int, rowEnd: Int, colStart: Int, colEnd: Int): Matrix[T] = {
    require(rowStart >= 0 && rowEnd <= _row && rowStart < rowEnd &&
  ...
  def sliceCols(colStart: Int, colEnd: Int): Matrix[T] = {
    require(colStart >= 0 && colEnd <= _col && colStart < colEnd,
  ...
  def isCompatible(that: Matrix[T]): Boolean = {
    this.row() == that.row() && this.col() == that.col() && this.gen().getClass == that.gen().getClass
  }
...
object Matrix {
  ...
  def apply[T <: Data](seq: Seq[Seq[T]])(implicit ev: Arithmetic[T]): Matrix[T] = {
    ...
    for (r <- 0 until row) {
      require(seq(r).length == col, s"Matrix row size mismatch at row $r: expected $col, got ${seq(r).length}")
...
object mat2x2 {
  ...
  def apply[T <: Data](a: Seq[T], b: Seq[T])(implicit ev: Arithmetic[T]): Matrix[T] = {
    require(a.length == 2 && b.length == 2, s"mat2x2 requires exactly 2x2 elements, got ${a.length}x${b.length}")
  ...
  def apply[T <: Data](seq: Seq[Seq[T]])(implicit ev: Arithmetic[T]): Matrix[T] = {
    require(seq.length == 2 && seq.forall(_.length == 2), s"mat2x2 requires exactly 2x2 elements, got ${seq.length}x${if (seq.nonEmpty) seq.head.length else 0}")
...
object mat3x3 {
  ...
  def apply[T <: Data](a: T, b: T, c: T, d: T, e: T, f: T, g: T, h: T, i: T)(implicit ev: Arithmetic[T]): Matrix[T] = {
  ...
  def apply[T <: Data](a: Seq[T], b: Seq[T], c: Seq[T])(implicit ev: Arithmetic[T]): Matrix[T] = {
    require(a.length == 3 && b.length == 3 && c.length == 3, s"mat3x3 requires exactly 3x3 elements, got ${a.length}x${b.length}x${c.length}")
  ...
  def apply[T <: Data](seq: Seq[Seq[T]])(implicit ev: Arithmetic[T]): Matrix[T] = {
    require(seq.length == 3 && seq.forall(_.length == 3), s"mat3x3 requires exactly 3x3 elements, got ${seq.length}x${if (seq.nonEmpty) seq.head.length else 0}")
...
object mat4x4 {
  ...
  def apply[T <: Data](a: T, b: T, c: T, d: T, e: T, f: T, g: T, h: T, i: T, j: T, k: T, l: T, m: T, n: T, o: T, p: T)(implicit ev: Arithmetic[T]): Matrix[T] = {
  ...
  def apply[T <: Data](a: Seq[T], b: Seq[T], c: Seq[T], d: Seq[T])(implicit ev: Arithmetic[T]): Matrix[T] = {
    require(a.length == 4 && b.length == 4 && c.length == 4 && d.length == 4,
  ...
  def apply[T <: Data](seq: Seq[Seq[T]])(implicit ev: Arithmetic[T]): Matrix[T] = {
    require(seq.length == 4 && seq.forall(_.length == 4), s"mat4x4 requires exactly 4x4 elements, got ${seq.length}x${if (seq.nonEmpty) seq.head.length else 0}")
